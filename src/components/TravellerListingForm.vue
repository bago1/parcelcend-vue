<template>
  <div>
    <h2>Create Traveller Listing</h2>
    <form @submit.prevent="createTravellerListing">
      <div>
...
export default {
  ...
  methods: {
    async createTravellerListing() {
      try {
        const response = await fetch("http://localhost:7799/api/listings/traveller", {
          method: "POST",
          headers: {
        ...
        if (response.ok) {
          this.responseMessage = "Traveller listing created successfully!";
        ...
        console.error("Error creating traveller listing:", error);
